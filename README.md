...
* Create Python 3.12 conda env named `cv2p312`
  ```
  conda update --all
  conda create --name cv2p312 python=3.12
  ```
* Terminal -> ***Activate conda env*** and install the latest `numpy`
  ```
  conda activate cv2p312
  pip install numpy
  ```
...
* Make ready & create WHL
  - Clone `git clone --single-branch --branch "cp312+cu121-linux" https://github.com/rathaumons/opencv-for-pyppbox.git`
