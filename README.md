...
* Requirements: `['numpy>=1.26.4; python_version=="3.9.*"']`
* CUDA & cuDNN are included in the package.
* The supported hardware for **Python 3.9** + **CUDA 12.1**:
  ```
...
* Create Python 3.9 conda env named `cv2p39`
  ```
  conda update --all
  conda create --name cv2p39 python=3.9
  ```
* Terminal -> ***Activate conda env*** and install the latest `numpy`
  ```
  conda activate cv2p39
  pip install numpy
  ```
...
* Make ready & create WHL
  - Clone `git clone --single-branch --branch "cp39+cu121-linux" https://github.com/rathaumons/opencv-for-pyppbox.git`
